...
#include <Wire.h>                      //tone library, download from url link (3)
...
void loop(void)
{
  ...
  vario=1000*((samples*N1)-N2*N3)/(samples*D1-D2*D2);
    ...
    String str_out =                                                                                  //combine all values and create part of NMEA data string output
      String("POV,")
      +String("P,")+String(dtostrf((float)Averaging_Filter(Pressure)/100,0,2,average_pressure_arr))+String(",") // Static Pressure
      ...
      +String("E,")+String(dtostrf(1000*((samples*N1)-N2*N3)/(samples*D1-D2*D2),0,2,vario_arr))+String(",")                                // Vario
